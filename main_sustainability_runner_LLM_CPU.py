...
def main():
    ...
    for col in ["CPU_hours", "FLOPs", "Model_Size_MB", "Parameters", "Avg_Output_Tokens_per_Query"]:
        if col not in merged_df.columns:
            merged_df[col] = 0.0
    ...
    merged_df["Hardware_Efficiency"] = (
        merged_df["Model_Accuracy"] / merged_df["CPU_hours"].replace(0, pd.NA)
    )
    ...
    merged_df["CPU_hours"] = merged_df["CPU_hours"].fillna(0.0)
    ...
    details_table.add_column("CPU Hours", justify="right")
    details_table.add_column("HWE (CPU-hr)", justify="right")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        for _, row in merged_df.iterrows():
            ...
            f.write(f"CPU Hours: {row['CPU_hours']:.6f}\n")
            f.write(f"HWE (Accuracy per CPU hour): {row['Hardware_Efficiency']:.6f}\n")
